...
欄位骨架（由左至右，寬度依實際欄位數調整，非固定死值）：
  時間 / 主鍵欄（如 HH:mm），font-mono；預設 text-xs text-neutral-mid（附屬資訊，不搶眼）
    字級例外：同列其他欄位為 text-sm 時，時間／筆數等附屬欄一律同步為 text-sm，
    主次改由顏色（text-neutral-mid）承擔而非字級——同一列出現兩種字級會像漏排版（見沖帳紀錄）
  ...
  欄位分組（欄位數 ≥ 6 時適用）：依語意分成識別群（時間／狀態／對象／筆數）／金額群／
    次要文字群，群內維持容器預設 gap-3、群間改用 ml-4～ml-6 拉開視覺區隔，讓眼睛能把
    同一群欄位讀成一組；不使用垂直分隔線（維持扁平原則，靠間距而非邊框分群）。
    表頭與資料列的欄寬＋群間外距必須逐欄一致，建議抽成共用常數（如 HISTORY_COL）供兩處
    import，避免其中一處改了欄寬另一處忘記同步而走鐘
...
Amount Cell：
  ...
  欄寬：附屬金額欄至少 w-28（112px）、主要（emphasis）金額欄至少 w-32（128px），
  確保六位數＋會計括號負值（如 `($10,000)`）在 font-mono text-sm 下不被擠壓換行

  負值：依全站通則（見上方 Reversed Settlement Panel 章節）改用會計括號表示法，
  括號與 $ 同色（emphasis 欄 text-neutral-mid、附屬欄 text-neutral-blue-gray），
  包在 $ 與數字外層（`(` 貼在 $ 前、`)` 貼在數字後），不破壞 tabular-nums 右緣對齊
...
Sticky 表頭（清單夠長、需捲動時適用）：
  欄位化表頭套用 sticky top-0 z-30 + 不透明背景（bg-surface-off-white，與頁面背景同色，
  資料列的 bg-white 才能乾淨捲到表頭底下而非透出）；表頭需給定固定高度（如 h-9）供下方
  分組標頭的 top 偏移計算。若清單另有分組標頭，分組標頭再吸一層 sticky top-9 z-20
  （top 值＝表頭固定高度），隨捲動被下一組標頭頂走。
  吸頂元素一律不可用 margin 留外距（如 mb-2），需併入自身 padding（如 pb-2）——sticky 元素
  的 margin 會在其下方形成透明縫隙，讓資料列從標頭縫隙中露出來。

  offset 基準：本專案桌機版面（≥ nav 1000px）預設無固定頂部列（Navbar 僅手機版 fixed），
  故一般清單 top 從 0 起算；若頁面另有固定頂部列（依該頁實際高度而定，不可憑感覺套用其他
  頁面的既有數值），才需疊加其高度。
```
...
**全站通則**：任何帶 `$` 的金額顯示，負值一律改用會計慣用括號表示法而非負號，
如 `-6300` 顯示為 `($6,300)`；由 `src/lib/utils.ts` 的 `fmtCurrency` 統一處理，系統內不存在
`-$6,300` 或 `$-6,300` 這兩種形式。上方「整面翻轉」的情境（本節開頭）是例外——那類面板傳入
`fmtCurrency` 前已先取絕對值，不會顯示括號。
...
|---|---|
| 時間序列（如逐日/逐週金額） | 垂直長條圖 |
| 時間序列且需同時呈現語意二分組成（如未收/已收、未付/已付） | 堆疊垂直長條圖，配色走 §11.4 語意色（不用 §11.3 類別序列），序列上限 2 段 |
