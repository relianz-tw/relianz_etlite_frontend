...
/** 桌機列金額欄：$ 固定貼齊欄位左緣、數字貼齊欄位右緣，比照 ReconTxnList 的 AmountCell 排版，
 *  讓同一欄內每列的 $ 對齊在同一直排；一律不顯示負號，無金額時顯示 0（比照其他金額欄格式，不特別弱化）。
...
function DeskAmountCell({ amount, emphasis }: { amount: number; emphasis?: boolean }) {
  return (
    <span
      className={cn(
        'flex shrink-0 items-center justify-between font-mono tabular-nums',
