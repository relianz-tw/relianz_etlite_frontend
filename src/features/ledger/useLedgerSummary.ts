...
import { buildTopShares, type ChannelShareDatum } from './summary';
 ...
 *     管道／廠商佔比、卡片頂部大數字（totals.primary）使用。
 *   - 已收／已付端點（receivables/collected/summary、payables/paid/summary，entry_date 口徑）：
 *     只取其 issuedVoucherAmount／receivedVoucherAmount 作為 totals.collected，供入帳狀況卡的第二個
    ...
    const request =
      side === 'sales'
        ? Promise.all([fetchReceivablesSummary(body), fetchReceivablesCollectedSummary(body)]).then(([main, collected]) => ({
            ...
            shares: buildTopShares(main.channelShares.map(c => ({ uuid: c.paymentChannelUuid, label: c.channelName, value: c.amount }))),
        ...
        : Promise.all([fetchPayablesSummary(body), fetchPayablesPaidSummary(body)]).then(([main, paid]) => ({
            dailyAmounts: main.dailyAmounts,
            shares: buildTopShares(main.vendorShares.map(v => ({ uuid: v.counterpartyUuid, label: v.counterpartyName, value: v.amount }))),
