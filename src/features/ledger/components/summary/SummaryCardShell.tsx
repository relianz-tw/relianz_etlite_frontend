...
export default function SummaryCardShell({ label, action, period, span, children }: SummaryCardShellProps) {
  return (
    <div
    ...
    >
      ...
      {period && <span className="mb-2.5 mt-0.5 block text-[11px] text-neutral-mid">{period}</span>}
      {/* min-h-0：抵銷 flex item 預設 min-height:auto，讓子項的 flex-1 能依卡片實際高度撐滿（grid stretch 出的高度）*/}
      <div className={`flex min-h-0 flex-1 flex-col ${period ? '' : 'mt-2.5'}`}>{children}</div>
    </div>
  );
}
