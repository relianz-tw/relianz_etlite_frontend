 ...
 *  應付薪資為前端即時加總，其餘減項欄位因無後端試算改為手動輸入（見計畫說明） */
export default function PayrollTable({ items, onChange, readOnly = false }: PayrollTableProps) {
  ...
  const handleCustomAmountChange = (employeeId: string, itemId: string, amount: number) => {
    onChange(
      items.map(item =>
        ...
        <thead>
          <tr>
            ...
            <th className={`${thDeduction} min-w-[150px]`}>
              勞退自提
              <div className="text-[11px] font-normal text-neutral-mid">可編輯</div>
            </th>
            <th className={`${thDeduction} min-w-[150px]`}>
              薪資扣繳稅款
              <div className="text-[11px] font-normal text-neutral-mid">員工負擔(可編輯)</div>
            </th>
            <th className={`${thDeduction} min-w-[150px]`}>
              勞保費(員工)
              <div className="text-[11px] font-normal text-neutral-mid">員工負擔(可編輯)</div>
