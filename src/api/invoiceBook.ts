 ...
 * 注意：實測 dev 環境這幾支 API 皆用 companyUuid 作為公司 uuid 參數名（api.md 舊版文件標示為 uuid，經 curl
 * 直接測試 dev.relianz.tw 確認以 companyUuid 才能通過後端必填欄位檢查）。
 */
import { apiFetch, buildQuery } from './client';
import { COMPANY_UUID } from './config';
import type {
  InvoiceBookListResult,
  InvoiceBookPeriodDto,
  SaveInvoiceBookBody,
  UpdateInvoiceBookBody,
} from './types';
...
/** 新增發票本（POST /ael/invoiceBook）；uuid 由後端產生，回應 data 固定為 null */
export function saveInvoiceBook(body: Omit<SaveInvoiceBookBody, 'companyUuid'>): Promise<null> {
  return apiFetch<null>('/ael/invoiceBook', {
    method: 'POST',
    body: JSON.stringify({ ...body, companyUuid: COMPANY_UUID }),
  });
}

/** 更新發票本（PATCH /ael/invoiceBook） */
export function updateInvoiceBook(body: Omit<UpdateInvoiceBookBody, 'companyUuid'>): Promise<null> {
  return apiFetch<null>('/ael/invoiceBook', {
    method: 'PATCH',
    body: JSON.stringify({ ...body, companyUuid: COMPANY_UUID }),
  });
